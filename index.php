...
<section class="container page">
	<div class="page-content">
		...
		<?php
			...
			if($page != 1){
		?>
			<a class="pagination_item" href="?page=<?=$page-1?><?=$cat_str?><?=$q_str?>"><?=$page?></a>
		<?php
			}
			for($i = 1; $i <= $count["total"]; $i++){
		?>
			<a class="pagination_item" href="?page=<?=$i?><?=$cat_str?><?=$q_str?>">Предыдущее</a>
		<?php
			}
			if($page != $count["total"]){
		?>
			<a class="pagination_item" href="?page=<?=$i?><?=$cat_str?><?=$q_str?>">Next</a>
		<?php
			}
		?>
